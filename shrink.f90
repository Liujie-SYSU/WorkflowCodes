...
!
        ...
        write(*,*) ' The output file after shrinking is only in .raw format.'
        write(*,*) ' ***In- & out-put names (without extension) should be listed in shk_list.dat'
        write(*,*) ' ***shk_list.dat fixed format: 30 & 30 characters for input & ouput per line.'
        write(*,*)
        open(10,file='shk_list.dat',status='old')
...
!
        ...
        DO K=1,MAX_Z
          DO J=1,MAX_Y
            DO I=1,MAX_X
             IF (LABEL(I,J,K).EQ.1) THEN
              IF (LABEL(I-1,J,K).EQ.0.OR.LABEL(I,J-1,K).EQ.0.OR.LABEL(I,J,K-1).EQ.0) LABEL(I,J,K)=-1
              IF (LABEL(I+1,J,K).EQ.0.OR.LABEL(I,J+1,K).EQ.0.OR.LABEL(I,J,K+1).EQ.0) LABEL(I,J,K)=-1
             ENDIF
            ENDDO
          ENDDO
...
!
        DO K=1,MAX_Z
          DO J=1,MAX_Y
            DO I=1,MAX_X
              IF (LABEL(I,J,K).EQ.-1) LABEL(I,J,K)=0
            ENDDO
          ENDDO
        ENDDO
!
        write(*,*) ' Output shrunk material-label-data : ',trim(OUTFL),'.raw'
